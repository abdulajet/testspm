...
@implementation SCLOngoingMediaCollection

-(instancetype)init {
    if(self = [super init]) {
        self.ongoingCallsArray = [NSMutableArray<SCLOngoingMedia *> new];
...
-(bool)addMedia:(SCLOngoingMedia *)media ForMember:(nonnull NSString *)memberId inConversation:(nonnull NSString *)conversationId {
...
-(void)removeMediaForMember:(nonnull NSString *)memberId inConversation:(nonnull NSString *)conversationId {
    ...
    if(index = [self getIndexForMember:memberId inConversation:conversationId]) {
        ...
        for (long i = [index longValue]; i < self.ongoingCallsArray.count; i++) {
            SCLOngoingMedia *mediaToUpdate =[self getMediaForIndex:[NSNumber numberWithLong:i]];
