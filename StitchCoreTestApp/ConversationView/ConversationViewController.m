...
@interface ConversationViewController ()<UIGestureRecognizerDelegate, UITextViewDelegate>
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receivedImageStatusEvent:)
                                                 name:@"imageStatusEvent"
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receivedMessageStatusEvent:)
...
- (void)receivedTextStatusEvent:(NSNotification *) notification {
    ...
    [self insertTextStatusEvent:statusEvent];
}
...
- (void)receivedImageStatusEvent:(NSNotification *) notification {
    ...
    [self insertImageStatusEvent:statusEvent];
}
...
- (void)addNewEvents:(NSMutableArray<NXMEvent *> *)events {
    for (NXMEvent *event in events) {
        if (event.type == NXMEventTypeMessageStatus) {
            NXMMessageStatusEvent *statusEvent = (NXMMessageStatusEvent *)event;
            NSInteger sequenceId = statusEvent.eventId;
            NSNumber *currentStatus = [self.messageStatuses objectForKey:@(sequenceId)];
            if (!currentStatus || [currentStatus integerValue] < statusEvent.status) {
                [self.messageStatuses setObject:@(statusEvent.status) forKey:@(sequenceId)];
            }
        }
    }
...
- (void)insertTextStatusEvent:(NXMMessageStatusEvent *)event {
...
- (void)insertImageStatusEvent:(NXMMessageStatusEvent *)event {
    if ([self isEventExists:event]) {
        return;
    }
