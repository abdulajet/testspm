...
-(void)loadEarlierEventsWithMaxAmount:(NSUInteger)maxAmount completion:(void (^_Nullable)(NSError * _Nullable error))completion {
    ...
    [self.stitchContext.coreClient getEventsInConversation:self.conversationDetails.uuid startId:startId endId:endId onSuccess:^(NSMutableArray<NXMEvent *> * _Nullable events) {
